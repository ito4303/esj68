...
\begin{align*}
N_{i} &\sim \mathrm{Pois}(\lambda_{i}) \\
C_{ij} &\sim \mathrm{Binom}(N_{i}, p_{ij})
\end{align*}

- $N$: サイト$i$の真の個体数量
- $\lambda_{i}$: サイト$i$の期待個体数量
- $C_{ij}$: サイト$i$の$j$回目の観測での観測値
- $p_{ij}$: サイト$i$の$j$回目の観測での観測確率
...
あるサイトで観測値$\{C_1,\dots,C_{J}\}$が得られる確率

\begin{align*}
\Pr(\{C_{1},\dots,C_{J}\} &\mid \lambda, \{p_{1},\dots,p_{J}\}) = \\
